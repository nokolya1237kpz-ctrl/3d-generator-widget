...
// 3D VIEWER (LAZY INIT)
// ============================================================================
function ensureViewerCanvasAttached() {
    const container = document.getElementById('preview-container');
    if (!container || !viewerRenderer) return false;

    if (!container.contains(viewerRenderer.domElement)) {
...
function init3DViewer() {
    if (viewerInitialized) return ensureViewerCanvasAttached();
...
const MODEL_CONFIGS = {
    ...
    'planter-stand': {
        name: 'Подставка под кашпо',
        params: [
            { id: 'outer_d', label: 'Внешний диаметр (мм)', type: 'number', min: 50, max: 220, step: 5, value: 90 },
            { id: 'inner_d', label: 'Внутренний диаметр (мм)', type: 'number', min: 20, max: 200, step: 5, value: 58 },
            { id: 'height', label: 'Высота подставки (мм)', type: 'number', min: 4, max: 40, step: 1, value: 8 },
            { id: 'legs', label: 'Количество ножек', type: 'number', min: 3, max: 8, step: 1, value: 4 },
            { id: 'rail', label: 'Толщина ребра (мм)', type: 'number', min: 4, max: 30, step: 1, value: 10 }
        ],
        hint: 'Можно автоматически подогнать под последнее кашпо: внешний диаметр чуть больше нижнего диаметра кашпо, внутренний — с запасом под дно.'
    },
    'shelf-bracket': {
...
function renderParams() {
    ...
    if (config.hint) {
        ...
        elements.paramsContainer.appendChild(hint);
    }

    if (modelType === 'planter-stand') {
        const actions = document.createElement('div');
        actions.className = 'inline-actions';
        actions.innerHTML = '<button type="button" class="btn-secondary" id="sync-stand-btn">Подогнать под кашпо</button>';
        elements.paramsContainer.appendChild(actions);
        setTimeout(() => {
            const btn = document.getElementById('sync-stand-btn');
            if (btn) btn.addEventListener('click', () => syncStandFromPlanter(true));
        }, 0);
    }
    ...
    if (modelType === 'planter-stand') syncStandFromPlanter(false);
}
...
function collectParams() {
    ...
    if (modelType === 'planter') {
        lastPlanterParams = { ...lastPlanterParams, ...params };
    }
    if (modelType === 'planter-stand' && params.inner_d >= params.outer_d - 8) {
        throw new Error('Внутренний диаметр подставки должен быть заметно меньше внешнего');
    }
    return params;
}
...
function syncStandFromPlanter(showMessage) {
    const bottom = Number(lastPlanterParams.bottom_d || 65);
    const wall = Number(lastPlanterParams.wall || 2.4);
    setFieldValue('outer_d', Math.round(bottom + 18));
    setFieldValue('inner_d', Math.max(20, Math.round(bottom - wall * 4)));
    setFieldValue('height', 8);
    setFieldValue('legs', 4);
    setFieldValue('rail', Math.max(6, Math.round(wall * 4)));
    if (showMessage) showStatus('✅ Подставка подогнана под последнее кашпо', 'success');
}

function applyPreset(name) {
    if (!elements.modelType) return;
    if (name === 'planter-small') {
        elements.modelType.value = 'planter';
        renderParams();
        setFieldValue('top_d', 72);
        setFieldValue('bottom_d', 52);
        setFieldValue('height', 62);
        setFieldValue('wall', 2.2);
        setFieldValue('drainage_holes', 4);
        lastPlanterParams = { top_d: 72, bottom_d: 52, height: 62, wall: 2.2, drainage_holes: 4 };
    } else if (name === 'planter-medium') {
        elements.modelType.value = 'planter';
        renderParams();
        setFieldValue('top_d', 105);
        setFieldValue('bottom_d', 74);
        setFieldValue('height', 92);
        setFieldValue('wall', 2.6);
        setFieldValue('drainage_holes', 6);
        lastPlanterParams = { top_d: 105, bottom_d: 74, height: 92, wall: 2.6, drainage_holes: 6 };
    } else if (name === 'stand-match') {
        elements.modelType.value = 'planter-stand';
        renderParams();
        syncStandFromPlanter(false);
    }
    hideResult();
}
...
async function handleGenerateClick() {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            showError('Сервер не ответил за 180 секунд. Попробуйте упростить параметры модели.');
        } else {
